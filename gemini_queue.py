...
def _record_pacing_feedback(result: dict):
    ...
    global _current_interval, _consecutive_clean_successes, _consecutive_rate_limits, _circuit_open_until
    ...
    if is_rate_limited:
        ...
        if _consecutive_rate_limits >= CIRCUIT_BREAKER_THRESHOLD and time.monotonic() >= _circuit_open_until:
            ...
            logger.error(
                "[CIRCUIT_BREAKER] Opening circuit for %.0fs after %d consecutive rate-limit failures",
                CIRCUIT_BREAKER_COOLDOWN_SECONDS, _consecutive_rate_limits,
            )
    else:
        _consecutive_rate_limits = 0
